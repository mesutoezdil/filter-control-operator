...
- Create a `text` file named `clar.txt`.
​
```bash
cat << EOF > clar.txt
clar:Road to reinvent yourself.
EOF
```

- Display the content of clar.txt.
​
```bash
cat clar.txt
```

- In the content of clar.txt, replace or translate aer letters with 'QAZ'.
​
```bash
cat clar.txt | tr aer QAZ
```
​
...
```
​
- Delete all the vowels in the content of clar.txt.
​
```bash
cat clar.txt | tr -d aeiou
```
​
- Write the whole content of clar.txt in capital letters.
​
```bash
cat clar.txt | tr [a-z] [A-Z]
```
...
​
- Display days.txt or write 'clar' on the screen, then write 'one'.
​
```bash
cat days.txt || echo clar ; echo one
```
...
- EXERCISE:
​
  1.a. Search for “clar.doc” in the current directory
    b. If it exists display its content
    c. If it does not exist print message “Too early!”
  2.Create a file named “clar.doc” that contains “Congratulations”
  3.Repeat Step 1
​
```bash
********************************
